         ...
         $category = DB::table('tbl_category')
          ...
          $bill_data = \App\BillMaster::select('bill_no')->orderBy('bill_no','desc')->first();
             ...
             $category = DB::table('tbl_category')
                ->select('tbl_category.*','tbl_type.type_name')
                ->leftjoin('tbl_type','tbl_type.type_id','=','tbl_category.type_id')
                ->where(['tbl_category.is_active' => '1','tbl_category.cid'=>$cid,'tbl_category.lid'=>$lid,'tbl_category.emp_id'=>$emp_id])
                ->orderBy('tbl_category.cat_name', 'asc')
                ->get();
    ...
    public function getItems()
    {
        ...
        if($cat_id==0)
        {
            ...
            foreach($item_data as $data)
            {
                ...
                $bdata.='<div class="col-sm-4 col-md-3"><button type="button" class="btn btn-block btn-default" style="border-color:#666666;" onclick="cal('.$data->item_final_rate.','.$data->item_id.')">'.$data->item_name.'<br/>'.$data->item_final_rate.'</button><input type="hidden" id="gitem_'.$data->item_id.'" value="'.$data->item_name.'"/></div>';
                $j++;
        ...
        else
        {
            if(Auth::guard('admin')->check()){
            $id = $this->admin->rid;
            $item_data = \App\Item::orderBy('item_name', 'asc')->where(['item_category'=>$cat_id,'cid'=>$id])->get();
            }else if(Auth::guard('web')->check()){
             $item_data = \App\Item::orderBy('item_name', 'asc')->where(['item_category'=>$cat_id])->get();   
            }
            else if(Auth::guard('employee')->check()){
            $cid = $this->employee->cid;
            $lid = $this->employee->lid;
            $emp_id = $this->employee->id;
            $item_data = \App\Item::orderBy('item_name', 'asc')->where(['item_category'=>$cat_id,'cid'=>$cid,'lid'=>$lid,'emp_id'=>$emp_id])->get();   
            }
            foreach($item_data as $data)
            {
                $replaced_item = str_replace(' ', '_', $data->item_name);
...
//           echo "<pre/>";print_r($bill_master);exit;
         foreach($requestData["stoppage"] as $data)
         {
             ...
             $requestData["item_totalrate"]=$data[4];
    ...
    public function checkItems()
    {
        ...
        if(Auth::guard('admin')->check()){
        $id = $this->admin->rid;
        $item_data = \App\Item::select('*')->where(['item_id'=>$code,'cid'=>$id])->first();
        }else if(Auth::guard('web')->check()){
           $item_data = \App\Item::select('*')->where(['item_id'=>$code])->first(); 
        }
         else if(Auth::guard('employee')->check()){
            $cid = $this->employee->cid;
            $lid = $this->employee->lid;
            $emp_id = $this->employee->id;
            $item_data = \App\Item::select('*')->where(['item_id'=>$code,'cid'=>$cid,'lid'=>$lid,'emp_id'=>$emp_id])->first(); 
         }
        if(empty($item_data))
        {
            if(Auth::guard('admin')->check()){
        $id = $this->admin->rid;
        $item_data = \App\Item::select('*')->where(['item_name'=>$code,'cid'=>$id])->first();
        }else if(Auth::guard('web')->check()){
           $item_data = \App\Item::select('*')->where(['item_name'=>$code])->first(); 
        }
         else if(Auth::guard('employee')->check()){
            $cid = $this->employee->cid;
            $lid = $this->employee->lid;
            $emp_id = $this->employee->id;
            $item_data = \App\Item::select('*')->where(['item_name'=>$code,'cid'=>$cid,'lid'=>$lid,'emp_id'=>$emp_id])->first(); 
         }
        }
