        ...
        if($cat_id==0)
        {
           ...
           $cid = $this->employee->cid;
            ...
            foreach($item_data as $data)
            {
               ...
               $bdata.='<div class="col-sm-2" style="width:20.83325%;"><button type="button" class="btn btn-block btn-default" style=" margin-left: -10px;border-color:#666666;width:auto;height:68px;margin-bottom:5px;" onclick="cal('.$data->item_final_rate.','.$data->item_id.','.$data->item_dis.','.$data->item_tax.')">'.wordwrap($data->item_name, 12, "<br/>", false).'<br/>Rs. '.$data->item_final_rate.'</button><input type="hidden" id="gitem_'.$data->item_id.'" value="'.$data->item_name.'"/></div>';
                $j++;
