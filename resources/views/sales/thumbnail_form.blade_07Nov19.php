...
<!--            <div class="box-header with-border">
              ...
              <div class="box-body">
                  <?php  foreach($category_data as $data){?>
                    <button type="submit" class="btn btn-success btn-lg cat" id="btn_submit_<?php echo $data->cat_id;?>" name="btn_submit" style="background-color:#00ffc3;color:black;margin-bottom:5px;font-weight:bold;width:96px;overflow: hidden;text-overflow: ellipsis;white-space:normal;margin-right:-4px;height:50px;" onclick="get_items(<?php echo $data->cat_id;?>,0)">{{$data->cat_name}}</button>
                    <span class="info-box-number cat_id" style="display:none;"><h2>{{$data->cat_id}}</h2></span>
                  
                  <?php }$all=0; ?>
                    <button type="submit" class="btn btn-success btn-lg" id="btn_submit1" name="btn_submit" style="background-color:#00ffc3;color:black;margin-top: -5px;font-weight:bold;;height:50px;" onclick="get_items(<?php echo $all;?>),0">All Items</button>
                    <br/>
                    <br/>
                    <div id="item_data">
...
//     window.print();
	  ...
	  // window.print();
        ...
        var printWindow = window.open(windowUrl, windowName, 'left=50000,top=50000,width=800,height=600');
        printWindow.document.write(printContent.innerHTML);
        printWindow.document.write('<html><head><style>th { font-size: 20px }.print{ font-size: 20px }.print_1{ font-size: 18px }.print{border:none}<style></head><body>');
        printWindow.document.close();
        printWindow.focus();
        printWindow.print();
        printWindow.close();
